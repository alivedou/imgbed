...
4. 在 **Build settings (构建设置)** 页面中进行如下配置：
   ...
   - **Build command (构建命令)**：填入 **`npx @opennextjs/cloudflare && mv .open-next/worker.js .open-next/_worker.js && cp -r .open-next/assets/_next .open-next/_next && cp _routes.json .open-next/_routes.json`**
